...
- (instancetype)init {
    if (self = [super init]) {
        [self getDatabaseFilePath];
        self.threadLock = [[NSRecursiveLock alloc] init];
    }
    return self;
}

- (void)getDatabaseFilePath {
    NSString *databasePath = [self databaseFilePath:@"EthercapApp.db"];
...
-(NSString*)createInsertSQL:(id)model tableName:(NSString*)tableName {
    ...
    for(NSString * key in dict.allKeys){
        ...
        valStr = [valStr stringByAppendingString:[NSString stringWithFormat:@",'%@'", [self convertCharacter:value]]];
    }
    if(colStr.length > 0){
...
-(NSString*)createUpdateSQL:(id)model tableName:(NSString*)tableName primarykeyName:(NSString *)primarykeyName {
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        id value = [dict objectForKey:key];
        if (i == dict.count - 1) {
            sql = [sql stringByAppendingString:[NSString stringWithFormat:@" %@ = '%@' ",key, [self convertCharacter:value]]];
        } else {
            if (i == 0) {
                sql = [sql stringByAppendingString:[NSString stringWithFormat:@"set %@ = '%@',",key, [self convertCharacter:value]]];
            } else {
                sql = [sql stringByAppendingString:[NSString stringWithFormat:@" %@ = '%@',",key, [self convertCharacter:value]]];
            }
        }
...
- (NSString *)convertCharacter:(id)value {
