...
- (NSArray *)descReadByField:(NSString *)field tableName:(NSString *)tableName {
    __block NSMutableArray *resultArray = [NSMutableArray new];
    if (![[HJDatabase shareInstance] isTableExist:tableName]) {
        return resultArray;
    }
    [[HJDatabase shareInstance] executeDB:^(FMDatabase *db) {
        NSString *sql = [NSString stringWithFormat:@"select * from %@ order by %@ desc",tableName,field];
        FMResultSet *rs = [db executeQuery:sql];
        while (rs.next) {
            NSMutableDictionary *dict = (NSMutableDictionary *)[rs resultDictionary];
            [dict removeObjectForKey:@"serial"];
...
- (NSString *)getMaxNumberFromTableName:(NSString *)tableName field:(NSString *)field whereDict:(NSDictionary *)whereDict {
    ...
    [[HJDatabase shareInstance] executeDB:^(FMDatabase *db) {
        ...
        result = [db stringForQuery:sql];
    }];
    return result;
}

- (NSString *)getMinNumberFromTableName:(NSString *)tableName field:(NSString *)field whereDict:(NSDictionary *)whereDict {
    __block NSString *result;
    if (![[HJDatabase shareInstance] isTableExist:tableName]) {
        return result;
    }
